...
# Use the command "python bellman_ford_1.py 'graph_name.csv' True" to run the program
...
def BellmanFord(graph, source) :
    ...
    for i in range(len(vertices) - 1) :
        # early termination condition: if graph is unchanged during one iteration of outer loop, terminate immediately
        ...
        for u,v,w in edges :
            ...
            if distance[u] != float('Inf') and distance[u] + w < distance[v] :
                ...
                changedThisIteration = True # record that update was made
                distance[v] = distance[u] + w
                predecessor[v] = u

        # break if no update was made this iteration
        if not changedThisIteration:
            break
